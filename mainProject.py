...
def is_valid(customer_account_number):
    ...
    else:  # if customer account  number is already available then this will return false. otherwise true.
...
def check_date(date):
    ...
    if (year > 2005 or year < 0) or (month > 12 or month < 1):
        return False
    else:
        if check_leap(year):
...
def display_account_summary(identity, choice):  # choice 1 for full summary; choice 2 for account balance.
    flag = 0
    customer_database = open("./database/Customer/customerDatabase.txt")
    output_message = ""
...
def delete_customer_account(identity, choice):  # choice 1 for admin, choice 2 for customer
    ...
    customer_database.write(data_collector) #Deleting the acc & Creating the txt DB again 

    conn=sqlite3.connect('Custumor_Database.db')
    cursor=conn.cursor()
    cursor.execute("delete from Customer_Details where Accno = ?",(identity,))
    conn.commit()
    conn.close()
...
def delete_admin_account(identity):
    ...
    if flag == 1:
        output_message = "Account id " + identity + " closed successfully!"
        print(output_message)
        adminMenu.printMessage_outside(output_message)
    else:
        output_message = "Account not found"
...
def change_PIN(identity, new_PIN):
    ...
    conn=sqlite3.connect('Custumor_Database.db')
    cursor=conn.cursor()
    cursor.execute("update Customer_Details set PIN = ? where Accno = ?",(new_PIN,identity))
    conn.commit()
    conn.close()
...
def check_credentials(identity, password, choice,
                      admin_access):  # checks credentials of admin/customer
...
class welcomeScreen:
    def __init__(self, window=None):
        ...
        window.title("Welcome to RVR BANK")
...
class CustomerLogin:
    def __init__(self, window=None):
        ...
        self.Button = tk.Button(Canvas1, text="Login", borderwidth="0", width=10, background="#00254a",foreground="#ffffff",
                                font="-family {Segoe UI} -size 10 -weight bold",
                                command=lambda: self.login(self.Entry1.get(), self.Entry1_1.get()))
        self.Button.place(relx=0.765, rely=0.755)

        self.Button_back = tk.Button(Canvas1, text="Back", borderwidth="0", width=10, background="#00254a",foreground="#ffffff",
...
class adminMenu:
    ...
    def printAccountSummary(identity):
        ...
        for widget in Frame1.winfo_children():
            widget.destroy()
        # getting output_message and displaying it in the output box
                    ...
                    if check_date(date_of_birth):
                        if is_valid_mobile(mobile_number):
                            if gender == "Male" or gender == "Female":
                                if nationality.__len__() != 0 and nationality.isalpha() and (nationality.islower() or nationality.istitle()):
                                    if KYC_document.__len__() != 0 and KYC_document.isalpha() and (KYC_document.islower() or KYC_document.istitle()):
                                        ...
                                        Error(Toplevel(self.master))
                                        Error.setMessage(self, message_shown="Invalid KYC!")
                                        return
                                else:
                                    Error(Toplevel(self.master))
                                    Error.setMessage(self, message_shown="Invalid Nationality!")
                                    return
                            else:
                                Error(Toplevel(self.master))
...
##        conn.close()
        ...
        #Adding data's in SQlite3 DB file
        conn=sqlite3.connect('Custumor_Database.db')
        cursor=conn.cursor()
        cursor.execute("insert into Customer_Details values(:Accno,:fullname,:type,:DOB,:mobileno,:gender,:nationality,:KYC,:PIN)",
                       {
                           'Accno':customer_account_number,
                           'fullname':name,
                           'type':account_type,
                           'DOB':date_of_birth,
...
class createAdmin:
    ...
    def create_admin_account(self, identity, password, confirm_password):
        if check_credentials(identity, "DO_NOT_CHECK_ADMIN", 1, False):
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="ID is available!")
        else:
